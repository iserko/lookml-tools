...
class BqWriter:
    ...
    def _write_to_csv(
        self, dataframe, target_bucket_name, bucket_folder, local_filename=None
    ):
        ...
        if not local_filename:
            ...
            filename = os.path.join(
                target_bucket_name, bucket_folder, BqWriter.FILENAME
            )
    ...
    def _upload_to_gcs(
        self, gcs_project_id, target_bucket_name, bucket_folder, filename
    ):
    ...
    def upload_to_bq(
        ...
        table_ref = bigquery_client.dataset(dataset).table(
            tablename
        )  # to target a partition concat a $date to the
