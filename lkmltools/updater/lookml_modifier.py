...
class LookMlModifier:
    ...
    def __init__(self, config):
        ...
        self.config = config
        definitions_provider = DefinitionsProviderFactory.instantiate(
            config["definitions"]["type"], config
        )
    ...
    def find_description(self, lookml, header_type, header_name):
        ...
        if header_type not in [
            FieldCategory.DIMENSION.value,
            FieldCategory.DIMENSION_GROUP.value,
            FieldCategory.MEASURE.value,
        ]:
    ...
    def modify(self, infilepath, outfilepath):
        ...
        if "use_basename" in self.config and self.config["use_basename"]:
            ...
            defs = self.definitions[
                self.definitions.file == os.path.basename(infilepath)
            ]
        else:
        ...
        for definition in defs.T.to_dict().values():
            ...
            description, has_key = self.find_description(
                lookml, definition["type"], definition["name"]
            )
            ...
            if has_key:
                logging.info(
                    "Existing description for %s.%s: '%s'",
                    definition["type"],
                    definition["name"],
                    description,
                )
                ...
                logging.info(
                    "No description for %s.%s", definition["type"], definition["name"]
                )
