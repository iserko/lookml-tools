...
class LookMlLinter:
    ...
    def write_file_csv(self, df):
        ...
        """
        df.to_csv(
            self.config["output"]["csv"]["file_output"],
            index=False,
            columns=["time", "file", "rule", "passed", "repo", "glob"],
        )
        logging.info(
            "File output written to %s", self.config["output"]["csv"]["file_output"]
        )
    ...
    def write_field_csv(self, df):
        ...
        """
        df.to_csv(
            self.config["output"]["csv"]["field_output"],
            index=False,
            columns=[
                "time",
                "file",
        ...
        logging.info(
            "Field output written to %s", self.config["output"]["csv"]["field_output"]
        )
    ...
    def run(self):
        ...
        for globstring in globstrings:
            ...
            for filepath in filepaths:
                ...
                if lookml.has_views():
                    v = lookml.views()[0]
                    field_out = self.run_field_rules(
                        v, "dimension", "dimensions", simple_filepath, field_out
                    )
                    field_out = self.run_field_rules(
                        v,
                        "dimension_group",
                    ...
                    field_out = self.run_field_rules(
                        v, "measure", "measures", simple_filepath, field_out
                    )
            ...
            [
                f.update({"glob": globstring})
                for f in field_out + file_out
                if not "glob" in f
            ]
