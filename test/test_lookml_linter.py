...
def test_run(monkeypatch):
    config = {
        ...
        "rules": {
            "file_level_rules": [{"name": "FilenameViewnameMatchRule", "run": True}],
            "field_level_rules": [
                {"name": "DescriptionRule", "run": True},
                {"name": "YesNoNameRule", "run": True},
            ],
...
def test_run_orphans():
    config = {
        "git": {"url": "https://github.com/exampleorg/examplerepo.git"},
        "infile_globs": ["test/test_orphans_repo/*.*.lkml"],
        "rules": {"other_rules": [{"name": "NoOrphansRule", "run": True}]},
        "output": {},
    }
    linter = LookMlLinter(config)
